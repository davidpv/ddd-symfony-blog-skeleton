...
final class RabbitMqDomainEventsConsumer
{
    ...
    {
        $this->sendMessageTo(\Shared\Infrastructure\Bus\Event\RabbitMq\RabbitMqExchangeNameFormatter::deadLetter($this->exchangeName), $envelope, $queue);
    }

    private function sendToRetry(AMQPEnvelope $envelope, AMQPQueue $queue): void
    {
        $this->sendMessageTo(\Shared\Infrastructure\Bus\Event\RabbitMq\RabbitMqExchangeNameFormatter::retry($this->exchangeName), $envelope, $queue);
    }
